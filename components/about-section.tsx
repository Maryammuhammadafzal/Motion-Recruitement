...
const AboutSection = () => {
  ...
  <div className="py-8 px-4 mx-auto max-w-7xl flex flex-col items-center sm:py-16 lg:px-6">
      <div className="max-w-3xl mb-8 lg:mb-16 flex flex-col items-center text-center">
         <Badge className='px-3 py-1 md:my-10 sm:my-6 my-3 bg-primary/20 text-primary md:text-sm text-xs'> Built for Long Term Success </Badge>
          <h2 className="mb-4 md:text-4xl sm:text-3xl text-2xl tracking-tight max-w-lg text-center font-semibold text-primary ">A Simple, Proven Hiring & Career Process</h2>
          <p className="max-w-2xl mb-10 font-light text-primary/70 lg:mb-8  md:text-base text-sm ">We remove complexity from tech hiring and job searching. Our structured, human first process ensures the right match faster, clearer, and with long term success in mind.</p>
      </div>
      <div className="space-y-8 md:grid md:grid-cols-2 lg:grid-cols-4 md:gap-12 md:space-y-0">
        {aboutData.map((data, index)=> (
          <div key={index} className='bg-white shadow-sm shadow-foreground rounded-2xl p-6'>
              ...
              <h3 className="mb-2 sm:text-xl text-lg font-medium text-primary">{data.title}</h3>
              <p className="text-primary/70 sm:text-sm text-xs">{data.description}</p>
          </div>
        ))}
